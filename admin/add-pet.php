...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($type) || empty($price)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO pets (name, type, breed, age, price, location, image_url, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->execute([$name, $type, $breed, $age, $price, $location, $image_url, $status]);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-submit:hover { transform: translateY(-2px); box-shadow: 0 5px 20px rgba(40,167,69,0.4); }
    ...
    <div class="container">
        <div class="form-card">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>Breed</label>
                    <input type="text" name="breed" value="<?php echo htmlspecialchars($_POST['breed'] ?? ''); ?>">
                </div>
                <div class="form-group">
                    <label>Age</label>
                    <input type="text" name="age" value="<?php echo htmlspecialchars($_POST['age'] ?? ''); ?>" placeholder="e.g., 2 Years">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="location" value="<?php echo htmlspecialchars($_POST['location'] ?? 'Bindura, Zimbabwe'); ?>">
                </div>
                <div class="form-group">
                    <label>Image URL</label>
                    <input type="text" name="image_url" value="<?php echo htmlspecialchars($_POST['image_url'] ?? ''); ?>" placeholder="https://images.unsplash.com/...">
                </div>
                <div class="form-group">
                    <label>Status</label>
                    <select name="status">
                ...
                <button type="submit" class="btn-submit"><i class="fas fa-save"></i> Add Pet</button>
            </form>
